...
TMP=$(sensors | grep temp1 | cut -c 16-19)

RFS=$(df / 2>/dev/null | awk '/root/ {printf "%.0f:%.0f:%s", $4*1024, $2*1024, $5}')
...
for i in $IFACES; do
	...
	if [ -n "$SSID" ] && [ "x$OFF" != "x1" ] && [ "x$OFF2" != "x1" ]; then
		...
		printf " | %-"$LINE"s |\n" "WLAN: $DEV mode: $MODE, ssid: $SSID, channel: $CHANNEL, conn: ${CNT:-0}"
	fi
done
